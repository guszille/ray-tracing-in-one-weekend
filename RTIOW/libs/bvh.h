...
public:
    ...
    bvh_node(std::vector<std::shared_ptr<hittable>>& objects, size_t start, size_t end)
    {
        ...
        else
        {
            ...
            {
                bbox = aabb(bbox, objects[index]->get_bounding_box());
            }
...
private:
    ...
    {
        const interval& a_axis_interval = a->get_bounding_box().axis_interval(axis_index);
        const interval& b_axis_interval = b->get_bounding_box().axis_interval(axis_index);
